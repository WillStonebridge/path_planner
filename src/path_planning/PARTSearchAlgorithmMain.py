...
def main():
    constAlt = 200
    fov = 40
    inputFile = "interop_example.json"
    ...
    createPoints(2, cells[3].searchPts, 1, feetStationaryObstacles, xWayPts, yWayPts, altWayPts, cameraWidth, constAlt)
    createPoints(2, cells[2].searchPts, 0, feetStationaryObstacles, xWayPts, yWayPts, altWayPts, cameraWidth, constAlt)
    createPoints(2, cells[1].searchPts, 0, feetStationaryObstacles, xWayPts, yWayPts, altWayPts, cameraWidth, constAlt)
    createPoints(2, cells[0].searchPts, 0, feetStationaryObstacles, xWayPts, yWayPts, altWayPts, cameraWidth, constAlt)
    wayPts = []
    ...
    for pt in searchGridPoints:
        lonGridPts.append(pt["longitude"])
        latGridPts.append(pt["latitude"])
        
    plt.figure(figsize = [5, 5])
    ax = plt.axes([0.1, 0.1, 0.8, 0.8], xlim=(minLon, maxLon), ylim=(minLat, maxLat))
    plt.plot(lonWayPts, latWayPts, color = "orange", marker = "o", markerfacecolor = "green", markeredgecolor = "green", linewidth = cameraWidth / gridLength * 5 * 0.8 * 72, markersize = 3) #area
    plt.plot(lonWayPts, latWayPts, color = "black", linewidth = 1)
    plt.plot(lonGridPts, latGridPts, color = "blue", linewidth = 3)
    
    
    for i in range(len(lonGridPts)):
        plt.text(lonGridPts[i], latGridPts[i], "B" + str(i))
    for i in range(len(stationaryObstacles)):
        ax.scatter([stationaryObstacles[i]["longitude"]], [stationaryObstacles[i]["latitude"]], s=(2 * stationaryObstacles[i]["radius"] / gridLength * 5 * 0.8 * 72) ** 2, c = "yellow", zorder = 3)
        plt.text(stationaryObstacles[i]["longitude"], stationaryObstacles[i]["latitude"], "O" + str(i))
    plt.gca().set_aspect('equal', adjustable = 'box')
    plt.xlabel("Horizontal (feet)")
    plt.ylabel("Vertical (feet)")
    plt.title("Map of boundary points, obstacles, and path")
