...
class PathPlanner:
    ...
    def calc_grid_index(self, node):
        return (node.lon ) * self.map.map_x_width + (node.lat )
    ...
    # uncomment to plot a star algorithm
            ...
            if current.lat == gnode.lat  and current.lon == gnode.lon:
        ...
        rough_path_x.append(self.map.transform_to_cart_position(gnode.lat))
        rough_path_y.append(self.map.transform_to_cart_position(gnode.lon))
        parent_index = gnode.parent_index
        
        while parent_index != -1:
            node = closed_set[parent_index]
            rough_path_x.append(self.map.transform_to_cart_position(node.lat, ))
            rough_path_y.append(self.map.transform_to_cart_position(node.lon, ))
    ...
    def verify_node(self, node):
        px = round(self.map.transform_to_cart_position(node.lat, ))
        py = round(self.map.transform_to_cart_position(node.lon, ))
        if px < 0: 
            return False
        elif py < 0:
            return False
            ...
            return False
        if self.map.obstacle_map[round(node.lat) - 1][round(node.lon) - 1]:
            return False
        return True
